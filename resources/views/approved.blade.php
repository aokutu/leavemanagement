    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        ...
        <div class="max-h-96 overflow-y-auto border border-gray-200 rounded-lg">
            ...
            <table class="min-w-full">
                ...
                <tr class="border-t">
                    <td class="px-4 py-2">👤 {{ $approvedleave['category'] }}</td>
                    <td class="px-4 py-2">{{ $approvedleave['employee'] }}</td>
                     <td class="px-4 py-2">{{ $approvedleave['approvedby'] }}</td>
                      <td class="px-4 py-2">{{ $approvedleave['from'] }}</td>
                        <td class="px-4 py-2">{{ $approvedleave['to'] }}</td>

                    <td class="text-center space-x-2">
                        <button onclick="editUser('{{ $approvedleave['id'] }}')" class="text-sky-600">📝</button>
                        <button onclick="deleteUser('{{ $approvedleave['id'] }}')" class="text-red-600">🗑️</button>
                    </td>
                </tr>
...
<div id="modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center">
    ...
    <div class="bg-white w-96 rounded-xl shadow-lg p-6 border border-sky-200">
        ...
        <h2 class="text-xl font-bold text-sky-700 mb-4">Create Employee</h2>
        ...
        <form action="/newemployee" method="POST">
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
                ...
                <label><input type="checkbox" name="create"> Create</label>
